...
const controller = {
    ...
    getThesisApps: async (req, res)=>{           
        try {
            ...
            logger.info('Loading thesis applications')
    ...
    approveThesisApp: async (req, res)=>{
        try{
            ...
            if (magicNum !== pdfNum){
                ...
                logger.info('File uploaded was checked to not be a ".PDF file based on file signature')
            }

            const maxSize = 1024 * 1024 * 1; // 1 for 1mb
            if (req.file.size > maxSize ){
                errors.push({ message: "Max upload size 1MB." });
                logger.info('Admin tried uploading a picture bigger than 1MB in updateProfilePicture');
            }
            ...
            logger.info('Approving a thesis application', {info: answer.rows[0]})
            if (errors.length> 0 ){
                try {
                    const results = await pool.query(
            ...
            if (buffer1 != null){
                ...
                pool.query(
                    ...
                    WHERE id = $2;`,[fileName, id , admin, date],(err, results)=>{
                        ...
                        logger.info('Successful approval of thesis budget application')
    ...
    rejectThesisApp: async (req, res)=>{
        try{
            ...
            logger.info('Thesis applications to be rejected', {info: answer.rows[0]})
            if (errors.length > 0 ){
                try {
                    const results = await pool.query(
            ...
            pool.query(
                ...
                WHERE id = $1;`,[id, admin, date],(err, results)=>{
                    ...
                    req.flash('success_msg', "User Verified");
                    logger.info('Successful rejection of a thesis budget application', {checked_by: admin})
    ...
    getApprovedThesis: async (req, res)=>{
        try {
            ...
            logger.info('Loading approved thesis applications', {info: applications})
            ...
            //logger.info('Loading approved thesis applications');
    ...
    getRejectedThesis: async (req, res)=>{
        try {
            ...
            logger.info('Loading rejected thesis applications', {info: applications})  
            return res.render('rejectedThesis', { applications });
        } catch (error) {
            if (process.env.MODE == 'debug'){
                globalLogger.error('Error fetching application')
            }
            console.error('Error fetching applications: ', error);
            return res.status(500).send('Internal Server Error');
        }
